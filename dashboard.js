...
(function () {
  ...
  function rodarCalculos(forcar) {
    ...
    setTimeout(() => {
      ...
      function finalizar() {
        ...
        resultado.cnfp = Object.entries(cnfpPorCategoria).map(([categoria, m2]) => ({ categoria, areaKm2: m2 / 1_000_000 }));
        resultado.ti = {
  ...
  function renderizar(r) {
    ...
    if (r.porBloco.length) {
      graficos.blocos = new Chart(document.getElementById("dash-chart-blocos"), {
        type: "bar",
        data: {
    ...
    if (r.porEntrega.length) {
      graficos.entregas = new Chart(document.getElementById("dash-chart-entregas"), {
        type: "bar",
        data: {
    ...
    if (r.cnfp.length) {
      graficos.cnfp = new Chart(document.getElementById("dash-chart-cnfp"), {
        type: "doughnut",
        data: {
          labels: r.cnfp.map(c => c.categoria),
          datasets: [{ data: r.cnfp.map(c => c.areaKm2),
            backgroundColor: r.cnfp.map(c => (coresCNFP[c.categoria] && coresCNFP[c.categoria].fill) || "#888") }]
        },
        options: { plugins: { legend: { display: false } }, maintainAspectRatio: false }
      });
    }

    document.getElementById("dash-tabela-cnfp").innerHTML = r.cnfp
      .slice()
    ...
    if (r.sicar.porTipo.length) {
      graficos.sicar = new Chart(document.getElementById("dash-chart-sicar"), {
        type: "bar",
        data: {
